...
function processPendingCharges() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield processNewPayments(); // PENDING → CONFIRMED
            yield processPayoutInitiated(); // CONFIRMED → PAYOUT_INITIATED
            yield processPayoutConfirmed(); // PAYOUT_INITIATED → PAYOUT_CONFIRMED → COMPLETED
        }
        catch (error) {
            ...
            try {
                ...
                if (paid) {
                    ...
                    yield prisma_client_1.prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (!updated.privKey) {
                            throw new Error(`Missing temp wallet privKey for charge ${updated.chargeId}`);
                        }
                        if (!((_a = updated.merchant) === null || _a === void 0 ? void 0 : _a.payoutStxAddress)) {
                            throw new Error(`Missing merchant payout address for charge ${updated.chargeId}`);
                        }
                    }));
                }
            }
            catch (error) {
                console.error(`❌ Error processing payment confirmation for charge ${charge.chargeId}:`, error);
                // Mark as failed for manual intervention
                yield markChargeFailed(charge.chargeId, `Payment confirmation failed: ${error}`);
            }
        }
    });
}
        ...
        for (const charge of confirmedCharges) {
            try {
                yield prisma_client_1.prisma.$transaction((tx) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const { txid } = yield (0, transferSbtc_1.transferSbtc)(updated.privKey, updated.address, updated.merchant.payoutStxAddress, updated.amount);
                    // Store the transaction ID
                    yield tx.charge.update({
                        where: { id: charge.id },
                        data: {
                            payoutTxId: txid,
                            lastProcessedAt: new Date(),
                    ...
                    console.log(`📤 SBTC payout initiated for charge ${charge.chargeId}, txid: ${txid}`);
                }));
            }
            catch (error) {
                console.error(`❌ Error initiating payout for charge ${charge.chargeId}:`, error);
                yield markChargeFailed(charge.chargeId, `Payout initiation failed: ${error}`);
            }
        }
    });
}
...
function processPayoutConfirmed() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log(`🔍 Found ${payoutInitiatedCharges.length} payouts to check for confirmation`);
        for (const charge of payoutInitiatedCharges) {
            try {
                ...
                if (txStatus.isSuccess) {
                    ...
                    const hasWebhook = !!((_a = updatedCharge.merchant) === null || _a === void 0 ? void 0 : _a.webhookUrl) &&
                        ...
                        try {
                            yield (0, deliverChargeWebhook_1.deliverChargeConfirmedWebhook)({
                                payload: {
                                    ...
                                    paidAt: (_c = updatedCharge.paidAt) === null || _c === void 0 ? void 0 : _c.toISOString(),
                    ...
                    yield markChargeFailed(charge.chargeId, `Payout transaction failed: ${txStatus.failureReason}`);
                    console.error(`❌ Payout failed for charge ${charge.chargeId}: ${txStatus.failureReason}`);
                }
                else if (txStatus.isPending) {
                    // Still pending - check if it's been too long
                    const timeSinceInitiated = Date.now() -
                        new Date(charge.lastProcessedAt || charge.createdAt).getTime();
                    const timeoutMs = 10 * 60 * 1000; // 10 minutes
                    if (timeSinceInitiated > timeoutMs) {
                        yield markChargeFailed(charge.chargeId, `Payout transaction timeout after ${timeoutMs / 1000 / 60} minutes`);
                        console.error(`⏰ Payout timeout for charge ${charge.chargeId}`);
                    }
                    else {
                        console.log(`⏳ Payout still pending for charge ${charge.chargeId} (${Math.round(timeSinceInitiated / 1000)}s elapsed)`);
                    }
                }
                // If status is "unknown", we'll try again next cycle
            }
            catch (error) {
                console.error(`❌ Error checking payout confirmation for charge ${charge.chargeId}:`, error);
            ...
            try {
                yield (0, deliverChargeWebhook_1.deliverChargeConfirmedWebhook)({
                    payload: {
                        ...
                        paidAt: (_c = charge.paidAt) === null || _c === void 0 ? void 0 : _c.toISOString(),
    ...
    return __awaiter(this, void 0, void 0, function* () {
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (hasEnough) {
                    console.log(`💰 Address ${address} has sufficient balance: ${balance} >= ${requiredAmount}`);
                }
                return hasEnough;
            }
            catch (error) {
                console.error(`❌ Attempt ${attempt} failed for balance check of ${address}:`, error === null || error === void 0 ? void 0 : error.message);
                if (attempt === MAX_RETRIES) {
                    console.error(`❌ Failed to check balance after ${MAX_RETRIES} attempts`);
                    return false;
                }
                // Exponential backoff
        ...
        for (const charge of stuckCharges) {
            ...
            if (charge.payoutTxId) {
                ...
                if (txStatus.isFailed) {
                    ...
                    console.log(`🔧 Recovered stuck charge ${charge.chargeId} - transaction was actually successful`);
                }
                // If still pending, let it continue normally
            }
            else {
                // No transaction ID - something went wrong during initiation
                yield markChargeFailed(charge.chargeId, "Recovery: Missing transaction ID after 30 minutes");
            }
        }
    });
}
// Enhanced poller with retry logic and exponential backoff
function startChargeProcessor() {
    let consecutiveFailures = 0;
    const maxFailures = 5;
    function pollWithRetry() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield processPendingCharges();
                consecutiveFailures = 0; // Reset on success
            }
            catch (error) {
                consecutiveFailures++;
                console.error(`💥 Poller error (failure #${consecutiveFailures}):`, error);
                if (consecutiveFailures >= maxFailures) {
                    console.error(`💥 Too many consecutive failures (${maxFailures}), extending delay`);
                }
            }
